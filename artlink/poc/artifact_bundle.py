...
from .packaging import Artifact, ArtifactManifestError, artifact_path, load_artifact_manifest
...
def is_hdl_source_artifact(artifact: Artifact) -> bool:
    return artifact.kind == "source" and (artifact.role in ("hdl-source", "generated-top") or artifact.language in HDL_SOURCE_LANGUAGES)
...
def _resolved_artifact(root: Path, artifact: Artifact) -> Artifact:
    return Artifact(
        path=artifact_path(root, artifact).resolve(),
        kind=artifact.kind,
        role=artifact.role,
...
def _normalized_artifact(artifact: Artifact) -> Artifact:
    ...
    return Artifact(
        ...
        kind=artifact.kind,
        role=artifact.role,
        language=source_language_from_path(artifact.path),
        format=artifact.format,
        modules=artifact.modules,
        media_type=artifact.media_type,
        sha256=artifact.sha256,
...
def _unsupported_source_language_errors(entries: tuple[ArtifactBundleEntry, ...]) -> tuple[str, ...]:
    ...
    for entry in entries:
        ...
        if artifact.language not in SUPPORTED_SOURCE_LANGUAGES:
            ...
            errors.append(f"unsupported source language for {artifact.path.as_posix()} from {entry.origin}: {language}; supported: {supported}")
    return tuple(errors)
...
def _duplicate_module_provider_errors(entries: tuple[ArtifactBundleEntry, ...]) -> tuple[str, ...]:
    ...
    for entry in entries:
        if entry.artifact.kind != "source":
            continue
        for module in dict.fromkeys(entry.artifact.modules):
            providers_by_module.setdefault(module, []).append(entry)
    return tuple(
        f"module {module} is provided by multiple artifacts: " + ", ".join(entry.artifact.path.as_posix() for entry in providers)
